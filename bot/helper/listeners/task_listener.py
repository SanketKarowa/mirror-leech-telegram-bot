...
from bot.helper.ext_utils.fs_utils import get_base_name, get_path_size, clean_download, clean_target, \
    is_first_archive_split, is_archive, is_archive_split, count_files_and_folders, get_mime_type, join_files
    ...
    async def onDownloadComplete(self):
        ...
        if self.isLeech:
            ...
            if ospath.isfile(file_path):
                ...
                mime_type = "Folder"
            new_path = f"{ospath.dirname(self.dir)}/" + "_".join(''.join(letter for letter in f"{up_name}" if letter.isalnum() or letter in [' ', '.', '-', '_']).split())
